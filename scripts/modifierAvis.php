    ...
    function modifierAvis(idAvis, prenom, note, commentaire) {
        Swal.fire({
            ...
            preConfirm: () => {
                ...
                const prenomInput = document.getElementById('modifierPrenomFormulaireAvis');
                const commentaireInput = document.getElementById('modifierCommentaireFormulaireAvis');

                // Force the first letter of name and comment to be uppercase
                prenomInput.value = capitalizeFirstLetter(prenomInput.value);
                commentaireInput.value = capitalizeFirstLetter(commentaireInput.value);

                // Regex patterns
                const regexPrenom = /^[A-Za-zÀ-ÖØ-öø-ÿ\s'-]+$/; // Prénom: lettres, espaces, accents, apostrophes, et tirets
                ...
                if (!regexPrenom.test(prenomInput.value)) {
                    Swal.showValidationMessage('Prénom invalide. Il peut contenir des lettres, espaces, accents, apostrophes, et tirets.');
                    return false;
                }
